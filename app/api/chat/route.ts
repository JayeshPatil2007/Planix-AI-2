...
import { teachingExplain, generateNotes } from '@/lib/gemini';
...
export async function POST(request: Request) {
  try {
    ...
    if (data.action === 'teaching') {
      const text = await teachingExplain(data.text, data.context, data.uploadedContext, data.chatHistory, data.userProgress);
      return NextResponse.json({ text });
    }

    if (data.action === 'notes') {
      const text = await generateNotes(data.text, data.context, data.uploadedContext, data.chatHistory, data.userProgress);
      return NextResponse.json({ text });
    }
